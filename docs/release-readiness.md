...
|----|----|
| 应用版本 | v2.2.6 |
| Git HEAD | `8b8427d4169c666e6e5ec0b3940773704133747b` |
| 相关提交（本次） | `8b8427d` docs(security): assign risk owner and update release readiness to honest state · `60ae7f6` chore(ci): add dependency security audit job · `ff6070a` docs(security): add deployment config, dependency tracker and release record |
| 验收时 Node / npm | **v22.23.0** / 10.9.8（Node.js 官方 portable 运行时） |
| 本地产物 EXE（server.exe）SHA256 | `74042E1571ACEEEA62533D54610DE7F50F2723344257405A18DA587B484206C5`（Node 22 干净依赖环境构建，非 CI 制品） |
| 本地产物 EXE（server.exe）SHA1 | `40A8036019140F8196176E68FFEA3847114FB85F` |
| 本地产物 EXE 大小 | 74,502,476 字节（≈71 MiB） |
| **CI 制品 EXE SHA256** | **待定**（本环境无法触发/读取 GitHub Actions 运行与下载产物，见 §6“环境验收状态”） |
| Docker 版本 | **不可用**（本环境未安装 Docker） |

> **重要**：发布候选提交**尚未推送到远程并触发 CI**。因此不存在“CI 运行链接、运行编号、CI 制品 EXE”。记录中的 EXE 哈希来自独立 Node 22 本地环境，证明 Node 22 门禁和制品可构建，但**不能代替发布候选的 CI 制品哈希**。正式发布仍应记录 CI 从同一发布候选提交构建的 EXE 哈希。

---

## 2. 构建/测试门禁结果（独立 Node 22 环境、pkg 目标 Node 22）
...
| `npm ci` | ✅ **通过**（Node v22.23.0 / npm 10.9.8，独立缓存与干净安装） |
| `npm audit --omit=dev` | ⚠️ **非零**（5 项：1 high + 4 low，按包聚合），见第四节风险接受 |

> Node 主版本要求（Task 1）已在官方 Node.js v22.23.0 portable 运行时中完成：`npm ci`、测试、lint、UI 构建、服务端构建和 EXE 冒烟测试全部通过。GitHub Actions 的运行链接、编号与 CI 制品哈希仍需在发布候选推送后回填。
...
| Node 22 本地门禁 | — | 已完成；CI 运行与制品追溯仍待发布候选推送后回填 | 发布前 |
...
> Node 22 独立环境门禁已在本机完成；Docker、真实 HTTPS 反向代理和真实 TeamSpeak 仍依赖外部基础设施。下表为**真实状态**，未做任何虚构。

| 任务 | 验收项 | 状态 | 原因 / 需在何处执行 |
|------|--------|------|--------------------|
| 1 | Node 22 门禁（`npm ci`/`test`/`lint`/`ui:build`/`server:build` + Windows EXE） | **本地通过；CI 追溯待回填** | Node v22.23.0 / npm 10.9.8 下全部通过，EXE SHA256 已记录；发布候选推送后仍需回填 CI 链接、运行号、提交 SHA、CI 制品 EXE SHA256 |
...
**本环境（自动可验证项）全部通过**：在 Node v22.23.0 的干净依赖环境中，服务端测试 **87/87**、前端 lint **0 问题**、UI 生产构建通过、Node 22 EXE 构建并独立启动通过 `/api/health`；Cookie 属性与 Origin 403 通过自动化测试。

**发布验收状态：`NOT RELEASE READY`（红色）**。Node 22 本地门禁已通过；Docker、真实 HTTPS/代理、真实 TeamSpeak 仍未执行，且发布候选提交尚未推送触发 CI、没有 CI 制品哈希。这些必须先在真实基础设施上完成并回填本记录，形成可追溯证据（CI 链接/运行号/提交 SHA、CI EXE SHA256、镜像 ID/摘要、HTTPS/WSS/代理 IP 结果、TeamSpeak 回归结果）。
